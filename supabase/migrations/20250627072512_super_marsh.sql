/*
  ...
  1. Data Cleanup
    - Delete all guest users and their associated data
    - Clean up any guest-related records

  2. Schema Changes
    - Drop the user_type column from users table
    - Remove user_type constraint
    - Update table comment

  3. Security Updates
    - Drop all existing RLS policies on users table
    - Create simplified policies for authenticated users only
    - Remove guest user policies from user_profiles
    - Revoke anonymous permissions

  4. Final Cleanup
    - Ensure all tables have proper RLS for authenticated users only
*/
