/*
  # Remove Guest User Functionality (Safe Migration)

  This migration safely removes guest user functionality from the database:
  
  1. Data Cleanup
     - Conditionally removes guest users only if user_type column exists
     - Uses DO blocks to avoid errors if column was already removed
  
  2. Schema Changes
     - Safely removes user_type column and constraints (if they exist)
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Delete other related data for guest users
