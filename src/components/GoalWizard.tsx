...
import { Wand2, Send, Sparkles, ArrowLeft, Mic, MicOff, Eye, Ear, AlertCircle } from 'lucide-react';
...
export const GoalWizard: React.FC<GoalWizardProps> = ({ onNavigate, user, appConfig, onFirstGoalCreated }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (hasOpenAI) {
        // Use real OpenAI assistant - pass the user ID and get taskCreated flag
        response = await sendToCrushion(inputValue, user.id, threadId);
        setThreadId(response.threadId);
        
        // Check if a task was created and trigger the callback
        if (response.taskCreated && onFirstGoalCreated) {
          console.log('[GoalWizard] Task was created, calling onFirstGoalCreated');
          onFirstGoalCreated();
        }
      } else {
        ...
        // Fallback responses never create tasks, so no need to call onFirstGoalCreated
      }
  ...
  return (
    <div className="flex flex-col h-screen bg-black text-white">
      ...
      {/* Input Area with Microphone - Always visible for continuous chat */}
